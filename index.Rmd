...
lecture_map <- function(fichier) {
  ...
  map <- map |> 
    ...
    mutate(
      ...
      `AE consommés` = round(`AE consommés`, digits = 0),
      `Solde réservations` = round(`Solde réservations`, digits = 0),
      `CP consommés` = round(`CP consommés`, digits = 0),
      `Blocage de fond` = round(`Blocage de fond`, digits = 0)
    )
  ...
  map <- map |> 
    ...
    ungroup() |> 
    mutate(
      `Liquidé` = `Liquidé EJ année N` + `Liquidé EJ antérieurs`,
...
calcul_depenses_map <- function(map) {
  ...
  bind_rows(missions, commandes, factures, non_identifiees) |> 
    mutate(
      engage = round(ae - paye - bloque, digits = 2),
      annee = as.integer(annee)
    ) |> 
    select(nature, annee, id, description, reserve, engage, paye, bloque)
}

depenses_map_brutes <- reactive(input$map_fichier |> lecture_map())
...
depenses <- reactive({
  ...
  dep <- bind_rows(dep_p, dep_ad, dep_ird_map)
  ...
  dep <- dep |> 
    ...
    relocate(nature, id, .after = last_col())
...
total_disponible <- reactive({
  ...
  total_hors_fg - reserve - engage - paye - bloque
})
...
proportion_consomme <- reactive({
  ...
  if (is.null(budget) || is.null(disponible)) return(NULL)
...
plot_conso_annuelle <- reactive({
  ...
  conso$type <- factor(
    ...
    labels = c("prévisionnel", "bloqué", "reservé", "engagé (non payé)", "payé")
  )
...
calcul_consommation_map <- function(map) {
  map |> 
    ...
    summarise(
      ...
      `Liquidé (Liquidés EJ année N + antérieurs)` = sum(`Liquidé`),
...
renderUI({
  ...
  if (!is.null(input$map_fichier) && is.null(depenses_map())) {
    tmp <- readxl::read_excel(input$map_fichier$datapath)
...
fileInput(
  "map_fichier",
  "Fichier Excel des dépenses exporté depuis MAP (optionnel)",
  accept = c(
    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    ".xlsx"
  ),
...
renderValueBox({
  ...
  if (is.null(prop)) {
    ...
    consomme <- budget_total_hors_fg() - total_disponible()
    value <- paste(
      consomme |> label_euro(),
...
renderUI({
  res <- c("<p />")
  total_hors_fg <- budget_total_hors_fg()
  if (!is.null(total_hors_fg)) {
    ventile <- saisie_config_ventilation() |> 
      pull(previsionnel) |> 
      sum(na.rm = TRUE)
    previ_non_ventile <- total_hors_fg - ventile
    res <- c(
      res,
  ...
  tbl |> 
    mutate(
      `Part consommée` = scales::percent(
        (`Prévisionnel` - `Total consommé`) / `Prévisionnel`,
        accuracy = .1
      ),
      Disponible = `Prévisionnel` - `Total consommé`
    ) |> 
...
**Modèle Excel pour les métadonnées :** [metadonnees_model.xlsx](https://github.com/larmarange/tbb/raw/main/metadonnees_modele.xlsx)
