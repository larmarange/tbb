...
import_config_ventilation <- reactive({
  if (is.null(metas()$nom) && is.null(depenses())) return(NULL)
  
  tbl_vide <- tibble(
    id_ventilation = NA_character_,
    rubrique = NA_character_,
    groupe = NA_character_,
    previsionnel = NA_real_,
    notes = NA_character_
  )
...
```{r}
renderUI({
  if(is.null(config_ventilation())) return(NULL)
  downloadButton("export_ventilation", "Exporter ventilation", class = "btn-primary")
})
...
### Saisie de la configuration des rubriques de ventilation
...
renderUI({
  res <- list(HTML(
    "Aucune dépense chargée."
  ))
  
  
  dep <- import_ventilation_depenses()
  if (is.null(dep)) return(res)
  ...
  res <- list(HTML(
    "<strong>Important :</strong> afin de ne pas perdre votre saisie, cliquez sur le <strong>bouton d'exportation</strong> (colonne de gauche). Le fichier Excel généré peut-être être ré-importé dans la colonne de gauche.<br />",
    "<strong style=\"color:red;\">Attention :</strong> tout import d'un fichier de ventilation ou modification de la configuration effacera les modifications non sauvegardées de la ventilation des dépenses.<br />",
    "<strong style=\"color:green;\">Astuce :</strong> si le défilement ne s'active pas correctement, vous pouvez cliquez deux fois sur le bouton <i>Exclure les dépenses nulles</i> pour forcer la page à afficher l'ascenseur de défilement.",
...
**Guide / Mode d'emploi :** [TBB_fiche_procedure.pdf](https://github.com/larmarange/tbb/raw/main/TBB_fiche_procedure.pdf)
